...
Claude API で Threads 投稿文を生成 → Threads API で自動投稿する。
...
def post_to_threads(post_text: str, dry_run: bool = False) -> str | None:
    ...
    if not token or not user_id:
        print("[ERROR] THREADS_ACCESS_TOKEN または THREADS_USER_ID が設定されていません。", file=sys.stderr)
        print("        --no-post オプションを使うか、環境変数を設定してください。", file=sys.stderr)
        sys.exit(1)
